...
<body class="bg-gray-100 font-sans">
    <div class="max-w-7xl mx-auto p-4">
        ...
        <div id="page2" class="tab-content bg-white p-6 rounded shadow">
            ...
            <div class="mb-2 flex justify-between text-sm font-medium"><span>测速进度</span><span id="progressText">0 / 0</span></div>
            <div class="w-full bg-gray-200 rounded-full h-4 mb-4"><div id="progressBar" class="bg-blue-600 h-4 rounded-full transition-all duration-300" style="width: 0%"></div></div>
    ...
    <script>
        ...
        async function parseSources() {
            ...
            if (text.startsWith('http')) {
                ...
                catch (e) { alert("跨域限制，请手动粘贴文本内容"); return; }
            }
            ...
            text.split('\\n').forEach(line => {
                ...
                if (line.startsWith('#EXTINF')) {
                    ...
                    let name = tempN || "未知"; let g = "其他";
                    for(let k in templates) if(templates[k].some(key => name.includes(key))) g = k;
                    if(g==="其他") g = tempG || "其他";
        ...
        async function fetchNetworkInfo() { const res = await fetch('/api/ip'); const d = await res.json(); document.getElementById('networkInfo').innerText = \`当前出口: \${d.ip} (\${d.country})\`; }
        ...
        async function startTesting() {
            ...
            const logDiv = document.getElementById('testLog'); logDiv.innerHTML = "🚀 开始测速...\\n";
            let tasks = []; rawGroups.forEach(g => { for(let n in g.channels) g.channels[n].urls.forEach(u => tasks.push({ g: g.name, n, u })); });
            const total = tasks.length;
            for(let i=0; i<tasks.length; i++) {
                if(!isTesting) break;
                const task = tasks[i]; const start = Date.now();
                try { const ctrl = new AbortController(); const tid = setTimeout(() => ctrl.abort(), 2000); await fetch(task.u.url, { mode: 'no-cors', signal: ctrl.signal }); clearTimeout(tid); task.u.time = Date.now() - start; } catch(e) { task.u.time = 9999; }
        ...
        function renderLinks(id) {
            ...
            const items = [{ k:'M3U', u:\`\${base}/sub/\${id}.m3u\` }, { k:'TXT', u:\`\${base}/sub/\${id}.txt\` }, { k:'EPG', u:\`\${base}/epg/\${id}.xml\` }];
            document.getElementById('linksContainer').innerHTML = items.map(i => \`
                <div class="flex items-center gap-2"><span class="w-10 font-bold text-xs text-gray-500">\${i.k}:</span><input class="flex-1 border p-2 text-xs rounded bg-white" value="\${i.u}" readonly><button onclick="navigator.clipboard.writeText('\${i.u}'); alert('已复制')" class="bg-blue-600 text-white px-4 py-2 rounded text-xs">复制</button></div>
            \`).join('');
        }
